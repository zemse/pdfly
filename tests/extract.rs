...
fn lorem_extracts_title_and_body() {
    ...
    assert!(page.runs.len() > 20, "expected many runs, got {}", page.runs.len());

    let all_text: String = page.runs.iter().map(|r| r.text.as_str()).collect();
    assert!(all_text.contains("Lorem Ipsum") || all_text.contains("Lorem Ipsum"), "title text present");
    assert!(all_text.to_lowercase().contains("dolor sit amet"), "body text present");
    ...
    let body_size = page.runs.iter().map(|r| r.font_size).fold(f64::MAX, f64::min);
    assert!(max_size > body_size * 2.0, "title font {max_size} should dwarf body {body_size}");
}
...
fn all_corpus_pdfs_extract_without_panic() {
    ...
    for entry in walk(&dir) {
        if entry.extension().and_then(|e| e.to_str()) == Some("pdf") {
            ...
            assert!(res.is_ok(), "failed to load {}: {:?}", entry.display(), res.err());
        }
    }
    assert!(count >= 10, "expected the bundled corpus, found {count} pdfs");
}
